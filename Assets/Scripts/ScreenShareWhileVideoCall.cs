...
namespace Agora_RTC_Plugin.API_Example.Examples.Advanced.ScreenShareWhileVideoCall
{
    public class ScreenShareWhileVideoCall : MonoBehaviour
    {
        ...
        public Transform ClientView;
        public Transform LocalScreen;
        public Transform ClientScreen;

        // Use this for initialization

        private void Awake()
        {
            Uid1 = (uint)UnityEngine.Random.Range(100, 999);
            Uid2 = (uint)UnityEngine.Random.Range(100, 999);
        }
        ...
        private void OnStartShareBtnClick()
        {
            ...
            if (option.Contains("ScreenCaptureSourceType_Window"))
            {
                ...
                var nRet = RtcEngine.StartScreenCaptureByWindowId(/*long.Parse(windowId)*/(long)0, default(Rectangle),
        ...
        public void Leave()
        {
            ...
            SceneManager.LoadScene("ScreenShareWhileVideoCallScene");
        }
        ...
        private static VideoSurface MakePlaneSurface(string goName )
        {
            ...
            var go = Instantiate(GameObject.Find("LocalView"));
            ...
            // set up transform
            if(goName == "MainCameraView")
            {
                go.transform.Rotate(-90.0f, 0.0f, 0.0f);
                go.transform.position = new Vector3(16.98f, 2.56f, -0.06f);
                go.transform.localScale = new Vector3(0.17f, 0.32f, .26f);
            }
            else if (goName == "ScreenShareView")
            {
                go.transform.Rotate(-90.0f, 0.0f, 0.0f);
                go.transform.position = new Vector3(13.57f, 2.6f, -0.03f);
                go.transform.localScale = new Vector3(0.45f, 0.31f, .23f);
            }
            else
            {
                go.transform.Rotate(-90.0f, 0.0f, 0.0f);
                go.transform.position = new Vector3(21.22f, 2.56f, -0.1f);
                go.transform.localScale = new Vector3(0.17f, 0.32f, .26f);
